...
class LocalDatabaseService implements DatabaseService {
  ...
  @override
  Future<void> clearData({@required dynamic boxId}) async {
    await _clearBox<Map>(boxId);
  }
  ...
  String _getBoxId(DatabaseBox box) => box.id;

  Future<Box<T>> _openBox<T>(DatabaseBox box) async {
    Box<T> _box;
  ...
  Future<void> _closeBox<T>(DatabaseBox box) async {
    Box _box;
    final boxId = _getBoxId(box);
  ...
  Future<void> _clearBox<T>(DatabaseBox box) async {
    try {
